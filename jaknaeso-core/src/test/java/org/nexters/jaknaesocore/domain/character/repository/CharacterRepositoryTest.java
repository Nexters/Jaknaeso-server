...
class CharacterRepositoryTest extends IntegrationTest {
  ...
  void 회원의_특정_캐릭터를_조회한다() {
    ...
    final Character actual =
        sut.findTopByMemberIdAndBundleIdAndDeletedAtIsNullWithMemberAndSurveyBundle(
  ...
  void 회원의_캐릭터_분석_결과를_조회한다() {
    ...
    final Character actual =
        sut.findByMemberIdAndBundleIdAndDeletedAtIsNullWithMemberAndSurveyBundle(
                member.getId(), bundle.getId())
            .get();

    then(actual)
        .extracting("characterValueReport")
