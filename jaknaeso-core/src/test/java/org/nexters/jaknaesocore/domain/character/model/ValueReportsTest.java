...
class ValueReportsTest extends IntegrationTest {
  ...
  @Test
  void 설문_번들의_키워드_점수_목록과_내가_제출한_설문_응답_목록을_바탕으로_가치관_리포트를_반환한다() {
    ...
    final List<Survey> surveys =
        surveyRepository.saveAll(List.of(survey1, survey2, survey3, survey4, survey5));
    ...
    final Map<Keyword, KeywordMetrics> keywordMetrics =
        calculateKeywordMetrics(Surveys.of(surveys));
    final List<ValueReport> actual = ValueReports.report(keywordMetrics, submissions);
    ...
    assertAll(
        ...
        () ->
            then(actual)
                .contains(ValueReport.of(ADVENTURE, ScaledBigDecimal.of(BigDecimal.valueOf(100)))),
        () ->
            then(actual)
