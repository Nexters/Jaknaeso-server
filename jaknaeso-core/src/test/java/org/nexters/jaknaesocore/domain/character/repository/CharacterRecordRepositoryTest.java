...
class CharacterRecordRepositoryTest extends IntegrationTest {
  ...
  void 회원의_캐릭터_분석_결과를_조회한다() {
    ...
    List<ValueReport> valueReports =
        valueReportRepository.saveAll(ValueReports.report(weightMap, metricsMap, submissions));
    characterRecord.updateValueReports(valueReports);

    final CharacterRecord actual =
        sut.findByIdAndDeletedAtIsNullWithValueReports(characterRecord.getId()).get();

    then(actual).extracting("valueReports").usingRecursiveComparison().isEqualTo(valueReports);
  }
}
